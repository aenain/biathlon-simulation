...
public class Scheduler extends NamedObject {
     ...
     * if the current Schedulable happens to be an external event or a sim-process.
     * 
	...
	void preemptSimProcess(EventNote preemptNote) {
		...
		EventNote currentNote = new EventNote(_currentProcess, null, presentTime(), Integer.MAX_VALUE);
		...
		if (_currentEvent != null) { // event-oriented
			...
			if (!_currentProcess.isTerminated()) {
				...
				if (_currentProcess.isReady()) {
					...
					// SimThread
					// executing the sim-process' lifeCycle method until
					// hold / passivate
					// or end of lifeCycle
				...
				myExperiment
						.sendWarning("Can not activate scheduled SimProcess! "
								...
								"The sim-process to be activated to take over control has "
										+ "already terminated.",
	...
	protected void schedule(Entity who, EventAbstract what, TimeSpan dt)
	{
		...
		EventNote note = new EventNote(who, what, time, priority);
	...
	protected void schedule(Entity who1, Entity who2, EventOf2Entities<?, ?> what, TimeSpan dt) {
		...
		EventNote note = new EventNote(who1, who2, what, time, what.getSchedulingPriority());
	...
	protected void schedule(Entity who1, Entity who2, Entity who3, EventOf3Entities<?, ?, ?> what, TimeSpan dt) {
		...
		EventNote note = new EventNote(who1, who2, who3, what, time, what.getSchedulingPriority());
	...
	protected void schedule(Entity who, EventAbstract what, TimeInstant when) {
		...
		EventNote note = new EventNote(who, what, when, priority);
	...
	protected void schedule(Entity who1, Entity who2, EventOf2Entities<?, ?> what, TimeInstant when) {
		...
		EventNote note = new EventNote(who1, who2, what, when, what.getSchedulingPriority());
	...
	protected void schedule(Entity who1, Entity who2, Entity who3, EventOf3Entities<?, ?, ?> what, TimeInstant when) {
		...
		EventNote note = new EventNote(who1, who2, who3, what, when, what.getSchedulingPriority());
	...
	protected void scheduleAfter(Schedulable after, Entity who, EventAbstract what) {
		...
		if (after != _currentSchedulable) {
			...
			evList.insertAfter(afterNote, new EventNote(who, what, afterNote
					.getTime(), afterNote.getPriority()));
			// sets the time equivalent to the Schedulable's
		} else {
			evList.insertAsFirst(new EventNote(who, what, presentTime(), Integer.MAX_VALUE));
		}
	...
	protected void scheduleAfter(Schedulable after, Entity who1, Entity who2, EventOf2Entities<?, ?> what) {
		...
		if (after != _currentSchedulable) {
			// all parameters checked, now go on and schedule
		    EventNote afterNote = after.getEventNotes().get(after.getEventNotes().size()-1);
			evList.insertAfter(afterNote, new EventNote(who1, who2, what, afterNote
					.getTime(), afterNote.getPriority()));
			// sets the time equivalent to the Schedulable's
		} else {
			evList.insertAsFirst(new EventNote(who1, who2, what, presentTime(), Integer.MAX_VALUE));
		}
	...
	protected void scheduleBefore(Schedulable before, Entity who, EventAbstract what) {
		...
		evList.insertBefore(beforeNote, new EventNote(who, what, beforeNote
				.getTime(), beforeNote.getPriority()));
		// sets the time equivalent to the Schedulable's
	}
	...
	protected void scheduleBefore(Schedulable before, Entity who1, Entity who2, EventOf2Entities<?, ?> what) {
		...
		// all parameters checked, now go on and schedule
	    EventNote beforeNote = before.getEventNotes().get(0);
		evList.insertBefore(beforeNote, new EventNote(who1, who2, what, beforeNote
				.getTime(), beforeNote.getPriority()));
		// sets the time equivalent to the Schedulable's
	}
