...
public class Res extends desmoj.core.simulator.QueueBased {
	...
	private static class UsedResources extends java.lang.Object {
		...
		/**
		 * The array of resources occupied by the sim-process. In fact the array
		 * is a java.util.Vector because one does not know how many resources
		 * the sim-process holds and the number of held resources might vary.
		 */
		...
		/**
		 * Returns the array of resources occupied by the sim-process.
		 * 
		...
		/**
		 * Sets the array of resources occupied by the sim-process to the given
	...
	/**
	 * Turns the deadlock check off. So whenever a sim-process can not get the
	...
	/**
	 * Turns the deadlock check on. So whenever a sim-process can not get the
	 ...
	 *         this time.
	 * @param sProc
	 *            SimProcess : The sim-process which is expected to hold some
	 *            Resources.
	 */
	protected int heldResources(SimProcess sProc) {
		...
		return j; // all the resources the sim-process holds at the moment
	}
	 ...
	 * moment the sim-process has to wait in a queue until enough products are
	 * available again.
	 * 
	...
	public boolean provide(int n) {
		...
		if (_passBy == false) {
			// see if the sim-process can be satisfied or has to wait in the
			// queue
		...
		// the Res provides all the resources the sim-process wants
		...
		// hand the resources over to the sim-process
		...
		// check for deadlock?
		if (getDeadlockCheck()) {
			// update the resourceDB: resources are assigned to the sim-process
			// now
	...
	public void takeBack(int n) {
		...
		// get the array of returned resources from the sim-process
	...
	protected void updateProvidedRes(SimProcess crntProcess,
		...
		// is the sim-process already holding resources?
		...
		for (int i = 0; i < _arrayOfUsedResources.size(); i++) {
			...
			if (procHoldRes.getProcess() == crntProcess) {
				...
				holdsResources = true; // the sim-process already holds
				// resources
			} // end if
		} // end for
