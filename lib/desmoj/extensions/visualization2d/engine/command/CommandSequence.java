...
public class CommandSequence {
	...
	private Vector<Command>			readCmdBuffer;
	private String					readCmd;
	private String					readCmdTmp;
	private Vector<Command>			writeCmdBuffer;
	...
	public CommandSequence(Model model, PrintWriter logFile){
		...
		this.readCmdBuffer 	= new Vector<Command>();
		this.readCmd		= "";
		this.readCmdTmp		= "";
		this.writeCmdBuffer = new Vector<Command>();
	...
	private long read(BufferedReader f, boolean init, long time) throws CommandException, ModelException, IOException{
		...
		Command last		= null;
		int		cmdCount	= 0;
		if(! this.readCmdBuffer.isEmpty()) last = this.readCmdBuffer.lastElement();
		...
		if(init || last == null || (last.getTime() <= time)){
			...
			do{
				...
				do{
					...
					this.readCmdBuffer.addElement(c);
					this.readCmdTmp	= this.readCmdTmp.substring(pos+1);
					pos = this.readCmdTmp.indexOf(Cmd.COMMAND_SEPARATOR);
				}while(pos != -1);
				last = this.readCmdBuffer.lastElement();
		...
		while(true){
			...
			if(init) 	condition = this.readCmdBuffer.firstElement().isInit();
			else		condition = this.readCmdBuffer.firstElement().getTime() <= time;
			if(! condition) break;
		...
		return this.readCmdBuffer.firstElement().getTime();
	}
