...
public class Entity implements Basic{
	...
	public Entity(Model model, String id, String entityTypeId , String state, long time) throws ModelException{
		...
		this.model			= model;
		this.id  			= id;
		this.name			= new Vector<Attribute>();
		...
		this.attribute		= new Hashtable<String, Vector<Attribute>>();
		this.velocity		= new Vector<Attribute>();
		this.velocity.add(new Attribute("velovity", Cmd.VELOCITY_KEY, time));
		this.priority		= new Vector<Attribute>();
		this.priority.add(new Attribute("priority", Cmd.PRIORITY_KEY, time));
		this.state			= new Vector<Attribute>();
		this.state.add(new Attribute("state", state, time));
		this.containerHistory	= new Vector<Attribute>();
	...
	public String getName(){
		return this.getAttribute(this.name.lastElement().getValue());
	}
	...
	public void setNameAttribute(String name, long time) throws ModelException{
		if(this.entityTyp.existPossibleAttribut(name)){
			Attribute last = this.name.lastElement();
	 ...
	 */
	public Vector<Attribute> getNameAttribute(){
		return this.name;
	}
	...
	public int getPriority(){
		int out = 0;
		String p = this.getAttribute(this.priority.lastElement().getValue());
		if(p != null){
			try{
	...
	public void setPriorityAttribute(String p, long time) throws ModelException{
		if(this.entityTyp.existPossibleAttribut(p)){
			Attribute last = this.priority.lastElement();
	 ...
	 */
	public Vector<Attribute> getPriorityAttribute(){
	...
	public double getVelocity(){
		double out = 1.0;
		String v = this.getAttribute(this.velocity.lastElement().getValue());
		if(v != null){
			try{
	...
	public void setVelocityAttribute(String v, long time) throws ModelException{
		if(this.entityTyp.existPossibleAttribut(v)){
			Attribute last = this.velocity.lastElement();
	 ...
	 */
	public Vector<Attribute> getVelocityAttribute(){
	...
	public String getState(){
		return this.state.lastElement().getValue();
	}
	 ...
	 */
	public Vector<Attribute> getStateHistory(){
		return this.state;
	}
	 ...
	 */
	public Vector<Attribute> getContainerHistory(){
	...
	public boolean isFree(){
		String key = this.containerHistory.lastElement().getKey();
	...
	public void setAttribute(String key, String value, long since){
		if(this.entityTyp.existPossibleAttribut(key)) {
			if(this.attribute.containsKey(key)){
				Attribute last = this.attribute.get(key).lastElement();
	...
	public String getAttribute(String key){
		String out = null;
		Vector<Attribute> v = this.attribute.get(key);
		if(v != null)	out = v.lastElement().getValue();
		return out;
	}
	 ...
	 */
	public Vector<Attribute> getAttributeHistory(String key){
