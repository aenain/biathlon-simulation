...
public class ResourceDB {
	 ...
	 * hashtable) a Vector holding all the pairs of a sim-process and the number
	 * of resources the sim-process holds at the moment (see the inner class
	 ...
	 * the inner class <code>RequestedResources</code>). The sim-process is
	...
	/**
	 * Stores all the sim-processes which are involved in a deadlock. Is filled
	...
	private static class AssignedResources extends java.lang.Object {
		...
		/**
		 * The sim-process using the resources at the moment.
		 */
		private SimProcess process;

		/**
		 * The number of resources seized by the sim-process.
		 */
		 ...
		 * 
		 * @param sProc
		 *            SimProcess : The sim-process holding the resources.
		 * @param seizedRes
		 *            int : The number of resources occupied by the sim-process.
		 */
		...
		/**
		 * Returns the sim-process which holds a number of resources.
		 * 
		 * @return SimProcess : The sim-process which holds a number of
		 *         resources.
		 */
		...
		/**
		 * Returns the number of resources seized by the sim-process.
		 * 
		 * @return int : The number of resources seized by the sim-process.
		 */
		...
		/**
		 * Sets the number of resources seized by the sim-process to the given
		 * value.
		 * 
		 * @param newQuantity
		 *            int : The new number of resources held by the sim-process.
		 */
	 ...
	 * number of resources requested by a sim-process. These pairs are stored in
	...
	*/
		...
		do // do while some reduction was made
		{
			...
			for (int i = 0; i < listOfProcs.size(); i++) {
				...
				if (reqRes == null) {
					...
					// delete the sim-process from the list
				...
				// if the sim-process can be satisfied in the future
	...
	/**
	 * This method is called when a sim-process can not get the resources desired
	 * to check if a possible deadlock situation has occured.
	 * 
	 * @param unsatProc
	 *            desmoj.SimProcess : The sim-process which can not get the
	...
	public synchronized boolean checkForDeadlock(SimProcess unsatProc) {
		...
		if (!checkProcess(unsatProc, _where))
			return false; // if the sim-process is not valid just return
	 ...
	 * resource pool (<code>Res</code> )). The sim-process and the number of
	 ...
	 * @param quantity
	 *            int : The number of resources the sim-process has used.
	 */
	public void deleteResAllocation(Res resPool, SimProcess doneProc,
		...
		if (!checkProcess(doneProc, _where))
			return; // if the sim-process is not valid just return
		...
		} else // get hold of the corresponding Vector
		{
						...
						if (quantity > assigRes.getSeizedUnits()) {
							sendWarning(
									"Attempt to delete more resources than "
											+ "the sim-process has allocated. Only all the formerly "
			...
			if (!foundInVec) {
				sendWarning(
						"Can't find the sim-process for which there should be "
						...
						"The entry in the database for the given SimProcess "
								+ "can not be found.",
						"Make sure to only delete resources which a sim-process "
	 ...
	 * @param quantity
	 *            int : The number of resources the sim-process gets.
	 */
	public void deleteResRequest(SimProcess gainProc, Res resPool, int quantity) {
		...
		if (!_requestTable.containsKey(gainProc)) { // if no entry -> error
			sendWarning(
					"Attempt to delete an entry in the resource database "
							+ "for a sim-process, but the sim-process does not exist in the "
							+ "database. The attempted action is ignored!",
					"ResourceDB Method: " + _where,
					"The given SimProcess has no entry in the database.",
					"Make sure that the given SimProcess has requested resources "
							+ "before you try to delete it's entry in the database.");
			return; // the sim-process is not registered in the resourceDB
		}
		...
		if (quantity > reqRes.getRequestedUnits()) {
			sendWarning(
					"Attempt to delete more requested resources than "
							+ "the sim-process has requested. Only all the formerly "
		...
		if ((reqRes.getRequestedUnits() == quantity)) {
			// remove the entry for the sim-process from the request hashtable
	...
	/**
	 * Makes an entry in the resource data base when a sim-process is allocating
	 ...
	 *            resources.
	 * @param quantity
	 *            int : The number of resources the sim-process is allocating.
	 */
	public void noteResourceAllocation(Res resourcePool,
		...
		if (_assignmentTable.containsKey(resourcePool)) // there is an entry ...
		{
			...
			for (int i = 0; i < arrayOfAssigResources.size(); i++) {
				...
				// is the sim-process already allocating resources of this
				// resource
				...
				if (alreadyAssigRes.getProcess() == allocatingProcess) {
					...
					// note that the sim-process is allocating resources already
					// from this
					// resource pool
	...
	/**
	 * Makes an entry in the resource data base when a sim-process is requesting
	 * a number of resources from a resource pool (<code>Res</code>). The
	 * resource pool and the number of requested resources will be saved in the
	 * request hashtable. The sim-process is the key to this hashtable.
	 * 
	 ...
	 *            resources.
	 * @param quantity
	 *            int : The number of resources the sim-process wants to get.
	 */
	public void noteResourceRequest(SimProcess requestingProcess,
		...
		{
			sendWarning(
					"Attempt to make a sim-process request resources from "
							+ "more than one resource pool. The attempted action is ignored!",
					"ResourceDB Method: " + _where,
					"A sim-process can request resource from only one resource pool, "
	 ...
	 * 
	 * @param process
	 *            desmoj.SimProcess : the sim-process which will return the
	...
	private void reduce(SimProcess process) {
		...
		// for every resource unit the sim-process holds
	 ...
	 * the sim-processes that are using resources from that resource pool. This
	...
	public String toHtmlString() {
		...
		if (_requestTable.isEmpty()) { // no SimProcess is requesting any
			...
			// loop through all the sim-processes waiting on resources
		...
		// list for every resource pool all the sim-processes holding his
		// resources
		buffer
