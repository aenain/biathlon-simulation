...
public abstract class ComplexSimProcess extends SimProcess {
	...
	/**
	 * The <code>Vector</code> holding all the sim-processes which are
	...
	/**
	 * Adds a sim-process as a component to this ComplexSimProcess. The
	 ...
	 * 
	 * @param compnt
	 *            desmoj.SimProcess : The sim-process to be added as a component
		...
		if (_components.contains(compnt)) {
			sendWarning(
					...
					"The given SimProcess is already a component of this "
							+ "ComplexSimProcess.",
					"Make sure the sim-process is not contained in the "
							+ "ComplexSimProcess already.");
			return;
		}

		// the sim-process to be added to a ComplexSimProcess should be active or
		// passivated but not scheduled. Just in case it is scheduled, cancel
		// that
		if (compnt.isScheduled()) {
			sendWarning(
					"The sim-process added to a ComplexSimProcess is scheduled! "
							+ "The scheduled activation of the sim-process will be "
							+ "cancelled.",
					"ComplexSimProcess: " + getName()
							+ " Method: synchronized void"
							+ " addComponent(SimProcess compnt)",
					"A sim-process added to a ComplexSimProcess is giving up its "
							+ "own lifeCycle and therefore should not be scheduled anymore."
							+ " It will be carried on by the lifeCycle of the "
							+ "ComplexSimProcess.",
					"Make sure that the sim-process is either adding itself to a "
							+ "ComplexSimProcess or that the sim-process is passive.");

			compnt.skipTraceNote();
			compnt.cancel();
		}

		// set the supervisor of the sim-process added
	 ...
	 * @param elem
	 *            desmoj.SimProcess : The sim-process which might be an element
	 ...
	 * recursively, until all simple SimProcesses are removed. The sim-processes
	...
	public synchronized void removeAllComponents() {
		...
		for (Enumeration<?> e = getComponents(); e.hasMoreElements();) {
			...
			// reset the supervisor of the sim-process element removed
			elem.setSupervisor(null);

			// the sim-process element is not blocked anymore
		...
		// remove the all the sim-process elements from the components Vector
	...
	/**
	 * Removes a sim-process from the elements of this ComplexSimProcess. The
	 ...
	 * 
	 * @param elem
	 *            desmoj.SimProcess : The sim-process to be removed from the
	...
	public synchronized void removeComponent(SimProcess elem) {
		...
		// the sim-process component must be contained in the container
		...
		// remove the sim-process element from the components Vector
